...
class TestBookingForm(TestCase):
    def test_form_is_valid(self):
        """Test for the Booking form is valid"""
        form = BookingForm({'start_time': '2024-10-31T23:16',
                            'number_of_guests': '2',
                            'special_request': 'Test Form'})
        self.assertTrue(form.is_valid(), msg='Form is not valid')

    def test_form_is_invalid(self):
        """Test for the Booking form is not valid, start_time is a required
        field and an empty string has been passed,
        that invalidates the form and the test comes back as True"""
        booking_form = BookingForm({'start_time': '',
                                    'number_of_guests': '2',
                                    'special_request': 'Test Form'})
